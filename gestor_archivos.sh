...
buscar_archivos() {
    ...
    if [ "$(id -u)" -eq 0 ]; then
    	comando+=" /"  # Root busca en todo el sistema
    else
    	comando+=" $HOME"  # Usuarios solo buscan en su home
    fi
    ...
    # Para usuarios normales, solo sus archivos (CORRECCIÓN: Cambiado el orden para que funcione correctamente)
    if [ "$(id -u)" -ne 0 ]; then
        comando+=" -user $(whoami)"
    fi
...
cambiar_permisos() {
    ...
    while IFS= read -r archivo; do
        if verificar_propietario "$archivo" && chmod "$permisos" "$archivo"; then
            ((exitos++))
        ...
        case "$opcion" in
            "1")
                if buscar_archivos; then
                    # CORRECCIÓN: Mostrar resultados primero y preguntar qué hacer
                    local archivos=$(cat "$TEMP_RESULTS")
                    zenity --text-info --title="Resultados de búsqueda" --filename="$TEMP_RESULTS" --width=800 --height=600
                    
                    local accion=$(zenity --list --title="Acción a realizar" \
                        --column="ID" --column="Acción" \
                        "1" "Cambiar permisos" \
                        "2" "No hacer nada")
                    
                    [ "$accion" = "1" ] && cambiar_permisos "$archivos"
                fi
                ;;
            "2")
